...
| `FILAMENT_BACKEND_INDX.md` | Bondtech INDX nozzle changer. NOT its own backend: a `toolchanger_addon` provider on `AmsBackendToolChanger` (no klipper-toolchanger, shared extruder/heater, subscription-sourced inventory). Read with `FILAMENT_BACKEND_TOOLCHANGER.md` |
