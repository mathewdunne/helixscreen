...
/**
 * @file test_print_status_filament_sensor_subject_lifetime.cpp
 * @brief PrintStatusPanel's FilamentSensorManager-subject observers must carry
 *        the manager's SubjectLifetime, or their guards dereference observer
 *        nodes that deinit_subjects() freed.
 *
 * FilamentSensorManager::deinit_subjects() runs subjects_.deinit_all(), which
 * frees every observer node on its subjects. It does not bump the
 ...
 * FilamentSensorManager::get_subjects_lifetime(). An observer created without
 * it calls lv_observer_remove() on a freed node when the panel is destroyed,
 * a double free of that node.
 */
...
class PrintStatusFilamentSensorLifetimeFixture : public LVGLUITestFixture {
  public:
    PrintStatusFilamentSensorLifetimeFixture() {
        // Publish the sensor subject names first: the panel resolves
        // "filament_sensor_count" by XML name, and a name registered in a scope
        // that has since been torn down resolves to nothing. init_subjects()
        // is idempotent on re-entry.
        ...
        subject_ = helix::FilamentSensorManager::instance().get_sensor_count_subject();
    ...
    ~PrintStatusFilamentSensorLifetimeFixture() override {
...
TEST_CASE_METHOD(PrintStatusFilamentSensorLifetimeFixture,
                 "PrintStatusPanel filament sensor observers survive manager deinit without "
    ...
    auto lifetime = helix::FilamentSensorManager::instance().get_subjects_lifetime();
    REQUIRE(lifetime != nullptr);
    REQUIRE(*lifetime);

    // The teardown path under test: frees every observer node on the
    // manager's subjects WITHOUT bumping the ObserverGuard invalidation
    // epoch.
    ...
    // subjects_.deinit_all() already freed.
    panel_release();
